...
import it.damose.dashboard.DashboardPanel;
...
public class MainWindow extends JFrame implements ConnectionListener {
    ...
    private void showDashboard() {
        // Crea finestra dashboard
        JDialog dashboardDialog = new JDialog(this, "Dashboard Real-Time", false);
        dashboardDialog.setSize(900, 700);
        dashboardDialog.setLocationRelativeTo(this);

        // Crea il pannello dashboard
        DashboardPanel dashboardPanel = new DashboardPanel(controller, realtimeManager);
        dashboardDialog.add(dashboardPanel);
    ...
    private void initLayout() {
        ...
        JPanel eastButtonsPanel = new JPanel(new GridLayout(1, 2, 5, 0));
        ...
        eastButtonsPanel.add(btnDashboard);
    ...
    private void showDetails(Object obj) {
        ...
        if (obj instanceof Stop s) {
            ...
            sb.append("───────────────────────────────────────\n");
            sb.append("(Clicca su una linea per visualizzarla)\n\n");
        ...
        // AGGIUNTA: Rendi il testo cliccabile
        makeRouteLinksClickable(obj);
    }

    /**
     * NUOVO METODO: Rende cliccabili le linee nel pannello dettagli
     */
    private void makeRouteLinksClickable(Object obj) {
        if (!(obj instanceof Stop stop)) {
            return; // Solo per fermate
        }

        detailArea.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                try {
                    int offset = detailArea.viewToModel2D(e.getPoint());
                    String text = detailArea.getText();

                    // Trova la riga cliccata
                    ...
                    // Controlla se è una riga con "→" (linea cliccabile)
                    if (line.startsWith("→ ")) {
                        // Estrai l'ID della route tra parentesi
                        int startParen = line.lastIndexOf('(');
                        int endParen = line.lastIndexOf(')');

                        if (startParen != -1 && endParen != -1) {
                            String routeId = line.substring(startParen + 1, endParen);

                            // Carica la route
                            Route clickedRoute = controller.getRouteById(routeId);
                            if (clickedRoute != null) {
                                System.out.println("Click su linea: " + clickedRoute.getName());

                                // Carica la route nella lista e mostra i dettagli
                                listModel.clear();
                                listModel.addElement(clickedRoute);
                                resultsList.setSelectedIndex(0);

                                // Mostra sulla mappa
                                showDetails(clickedRoute);

                                // Centra mappa sulla linea
                                if (mappa != null) {
                                    mappa.setFilteredRoute(clickedRoute);
                                }
                            }
                        }
                    }
        ...
        // Cambia cursore quando si passa sopra una linea
        detailArea.addMouseMotionListener(new MouseMotionAdapter() {
            @Override
            public void mouseMoved(MouseEvent e) {
                try {
                    ...
                    if (line.startsWith("→ ")) {
                        detailArea.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
                    } else {
                        detailArea.setCursor(Cursor.getDefaultCursor());
                    }
                } catch (Exception ex) {
                    detailArea.setCursor(Cursor.getDefaultCursor());
                }
